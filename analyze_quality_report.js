...
const fileGroups = {
    ...
    'Data Models': { files: [], failed: 0, errors: {}, scores: [], filter: spec => spec.type === 'data_model' || spec.id.includes('data_model') },
    'Frontend Specs': { files: [], failed: 0, errors: {}, scores: [], filter: spec => spec.category === 'frontend' || spec.id.includes('frontend') },
    'Documentation': { files: [], failed: 0, errors: {}, scores: [], filter: spec => spec.category === 'documentation' || spec.type === 'documentation' }
};
    ...
    try {
        ...
        const results = {
            summary: {
                totalFiles,
                totalFailed,
                passRate: totalFiles > 0 ? ((totalFiles - totalFailed) / totalFiles * 100).toFixed(2) : '0.00'
            },
            groups: {},
            topErrors: []
        };
        ...
        for (const [groupName, group] of Object.entries(fileGroups)) {
            ...
            const passRate = group.files.length > 0
...
function generateRecommendations(errorType) {
    const recommendations = {
        'pattern_mismatch: Content must be at least 10 characters long': 'Expand the content to meet minimum length requirements. Add more detailed descriptions and explanations.',
        'pattern_mismatch: Content must follow markdown format': 'Ensure content follows proper markdown formatting. Use headers, lists, and other markdown elements correctly.',
        'pattern_mismatch: Missing required sections': 'Add all required sections as per the specification template. Check the specification guidelines for required sections.',
        'pattern_mismatch: Invalid structure': 'Restructure the content to match the expected format. Refer to the specification template for correct structure.',
        'pattern_mismatch: Title must follow pattern': 'Update the title to follow the required naming convention. Check the specification guidelines for title format.',
        'WARNING: incomplete_content: Functional requirement should use clear language (shall, must, should, will)': 'Rewrite requirements using clear modal verbs (shall, must, should, will) to make them unambiguous.',
        'WARNING: unclear_requirement: Functional requirement should use clear language (shall, must, should, will)': 'Use modal verbs to make requirements unambiguous and clearly state what the system must do.',
        'WARNING: incomplete_content: Specification missing author information': 'Add author information to track ownership and responsibility for the specification.',
        'WARNING: incomplete_content: Missing version information': 'Add version information to track changes and maintain document history.',
        'WARNING: incomplete_content: Missing status information': 'Specify the current status of the specification (draft, review, approved, etc.).',
        'missing-required-field': 'Ensure all required fields are included in the specification. Check the specification template for required fields.',
        'invalid-format': 'Verify that the content follows the correct format. Refer to the documentation for proper formatting guidelines.',
        'missing-section': 'Add the missing sections to complete the specification. Use the specification checklist to ensure all sections are included.',
        'empty-content': 'Provide meaningful content for empty sections. Specifications should contain detailed information, not placeholders.',
        'syntax-error': 'Fix syntax errors in the markup or code. Validate the content using appropriate linters or validators.',
        'inconsistent-naming': 'Use consistent naming conventions throughout the specification. Follow the project naming guidelines.',
        'missing-dependency': 'Specify all required dependencies and relationships. Ensure all referenced components are properly documented.',
        'validation-failed': 'Address validation errors by ensuring the content meets all quality standards and requirements.',
        'duplicate-content': 'Remove or consolidate duplicate content. Each section should provide unique information.',
        'incomplete-specification': 'Complete the specification by adding all required details and missing information.'
    };
    
    return recommendations[errorType] || 'Review the error details and consult the specification guidelines for resolution.';
}
...
function main() {
    ...
    for (const [groupName, group] of Object.entries(results.groups)) {
        console.log(`| ${groupName} | ${group.totalFiles} | ${group.failed} | ${group.passRate}% | ${group.avgScore} | ${group.errorTypes} |`);
    }
