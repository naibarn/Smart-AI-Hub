...
interface AutoTopupSettings {
  ...
  amountStrategy: FixedAmountStrategy | PercentageAmountStrategy | 
                  TieredAmountStrategy | DynamicAmountStrategy;
...
async function handlePaymentMethodExpiration(): Promise<void> {
  ...
  for (const method of expiringMethods) {
    ...
    if (autoTopupSettings.enabled && 
        autoTopupSettings.paymentMethod.identifier === method.id) {
...
describe('Account Status Changes', () => {
  test('Disables auto top-up when account is suspended', async () => {
    ...
    // Verify user was notified
    expect(notifyAutoTopupDisabled).toHaveBeenCalledWith(
      user.id,
      'suspended'
    );
  });
