...
async function main() {
  ...
  const adminPermissions = createdPermissions.filter(
    (p: Permission) => !p.name.includes('superadmin') && p.name !== 'roles:assign' // Admin can't assign roles, only superadmin can
  );
  await Promise.all(
  ...
  const guestPermissions = createdPermissions.filter(
    (p: Permission) => p.name === 'services:use' // Guests can only use services
  );
  await Promise.all(
