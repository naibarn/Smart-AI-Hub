...
async function main() {
    ...
    const adminPermissions = createdPermissions.filter((p) => !p.name.includes('superadmin') &&
        p.name !== 'roles:assign' // Admin can't assign roles, only superadmin can
    );
    await Promise.all(adminPermissions.map((permission) => prisma.rolePermission.upsert({
        where: {
            roleId_permissionId: {
                roleId: adminRole.id,
                permissionId: permission.id,
    ...
    const managerPermissions = createdPermissions.filter((p) => p.name === 'users:read' ||
        p.name === 'credits:read' ||
        p.name === 'credits:update' ||
        p.name === 'credits:adjust' ||
        p.name === 'services:read' ||
        p.name === 'roles:read');
    await Promise.all(managerPermissions.map((permission) => prisma.rolePermission.upsert({
        where: {
            roleId_permissionId: {
    ...
    const userPermissions = createdPermissions.filter((p) => p.name === 'users:read' ||
        p.name === 'credits:read' ||
        p.name === 'services:use' ||
        p.name === 'services:read');
    await Promise.all(userPermissions.map((permission) => prisma.rolePermission.upsert({
        where: {
            roleId_permissionId: {
                roleId: userRole.id,
                permissionId: permission.id,
    ...
    const guestPermissions = createdPermissions.filter((p) => p.name === 'services:use' // Guests can only use services
    );
    await Promise.all(guestPermissions.map((permission) => prisma.rolePermission.upsert({
        where: {
            roleId_permissionId: {
                roleId: guestRole.id,
                permissionId: permission.id,
