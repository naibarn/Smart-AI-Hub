...
import { mockCreditService } from '../__mocks__/services.mock';
...
describe('Credit Controller', () => {
  ...
  describe('GET /credits/balance', () => {
    ...
    test('should return cached balance when available', async () => {
      ...
      (mockRedisService.get as jest.Mock).mockResolvedValue('100');
  ...
  describe('GET /credits/history', () => {
    test('should return paginated history', async () => {
      ...
      (mockCreditService.getHistory as jest.Mock).mockResolvedValue(mockHistory);
      ...
      expect(mockCreditService.getHistory).toHaveBeenCalledWith('test-user-id', 1, 10);
    });
    ...
    test('should use default pagination params', async () => {
      ...
      expect(mockCreditService.getHistory).toHaveBeenCalledWith('test-user-id', 1, 20);
    });
  ...
  describe('POST /credits/redeem', () => {
    test('should redeem valid promo code', async () => {
      ...
      expect(mockCreditService.redeemPromo).toHaveBeenCalledWith('test-user-id', 'TEST123');
    ...
    test('should return error for invalid code', async () => {
      ...
      expect(response.status).toBe(500);
      expect(mockCreditService.redeemPromo).toHaveBeenCalledWith('test-user-id', 'INVALID');
    });
    ...
    test('should return error if already used', async () => {
      ...
      expect(response.status).toBe(500);
      expect(mockCreditService.redeemPromo).toHaveBeenCalledWith('test-user-id', 'USED123');
    });
