...
function migrateSpec(filePath, templateType = CONFIG.defaultTemplate) {
  try {
    ...
    const migratedContent = generateMigratedSpec(
      frontmatter,
      mappedSections,
      templateType,
      templateContent
    ...
    return {
      ...
      sectionsAdded:
        Object.keys(generateAllSectionNames(templateContent)).length -
        Object.keys(mappedSections).length,
    };
  } catch (error) {
...
function detectTemplateType(filePath, content) {
  ...
  if (
    ...
    pathLower.includes('fr_') ||
    content.includes('functional requirement') ||
