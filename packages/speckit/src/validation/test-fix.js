...
async function testValidationFix() {
  ...
  try {
    ...
    const contentLengthError = validationResult.errors.find((e) =>
...
---`,
    ...
    const frontMatterError = frontMatterResult.errors.find((e) =>
      e.message.includes('Content must be at least 10 characters long')
    );
    ...
    const shortError = shortResult.errors.find((e) =>
      e.message.includes('Content must be at least 10 characters long')
    );
    ...
    const emptyError = emptyResult.errors.find((e) => e.type === 'missing_field');
