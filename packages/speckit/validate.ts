...
async function validateDatabase(
  config: ValidationConfig['database']
...
async function validateServices(
  config: ValidationConfig['services']
): Promise<{ found: number; total: number; details: any[] }> {
  if (!config || !config.enabled) {
    return { found: 0, total: 0, details: [] };
  }
...
async function validateFrontend(
  config: ValidationConfig['frontend']
): Promise<{ found: number; total: number; details: any[] }> {
  if (!config || !config.enabled) {
    return { found: 0, total: 0, details: [] };
  }
...
async function main() {
  ...
  for (const spec of config.specs) {
    ...
    console.log(
      `  Path: ${spec.path} ${specExists ? colorize('✅', 'green') : colorize('❌', 'red')}`
    );
    console.log(
      `  Backlog: ${spec.backlog} ${backlogExists ? colorize('✅', 'green') : colorize('❌', 'red')}`
    );
    ...
    console.log(
      colorize(
        `\n  Progress: ${totalFound}/${totalExpected} (${progress}%)`,
