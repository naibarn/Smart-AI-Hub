...
test.describe('Visibility Middleware Verification', () => {
  ...
  test('Agency cannot see other Agencies', async ({ request }) => {
    ...
    const otherAgencies = data.members.filter((m: any) => 
      m.tier === 'agency' && m.id !== currentAgencyId
    );
    expect(otherAgencies.length).toBe(0);
    
    // Should see own organizations
    const ownOrgs = data.members.filter((m: any) => 
      m.tier === 'organization' && m.agencyId === currentAgencyId
    );
  ...
  test('Organization cannot see other Organizations', async ({ request }) => {
    ...
    const otherOrgs = data.members.filter((m: any) => 
      m.tier === 'organization' && m.id !== currentOrgId
    );
    expect(otherOrgs.length).toBe(0);
    
    // Should see own admins and generals
    const ownMembers = data.members.filter((m: any) => 
      (m.tier === 'admin' || m.tier === 'general') && m.organizationId === currentOrgId
    );
  ...
  test('visibility middleware is applied to hierarchy tree endpoint', async ({ request }) => {
    ...
    const otherAgencies = data.tree.filter((node: any) => 
      node.type === 'agency' && node.id !== currentAgencyId
    );
    expect(otherAgencies.length).toBe(0);
  });

  test('visibility middleware prevents access to user details of invisible users', async ({ request }) => {
  ...
  test('visibility middleware logs access attempts', async ({ request }) => {
    ...
    const accessLog = logs.find((log: any) => 
      log.action === 'HIERARCHY_MEMBERS_ACCESS' && 
      log.userId === 'agency-a-user-id'
    );
  ...
  test('visibility middleware handles pagination correctly', async ({ request }) => {
    ...
    const response = await request.get('http://localhost:3001/api/v1/hierarchy/members?page=1&limit=10', {
      headers: {
        'Authorization': `Bearer ${agencyAToken}`
      }
    });
    ...
    const otherAgencyUsers = data.members.filter((m: any) => 
      m.tier === 'agency' && m.id !== currentAgencyId
    );
    expect(otherAgencyUsers.length).toBe(0);
  });
  ...
  test('visibility middleware handles sorting correctly', async ({ request }) => {
    ...
    const response = await request.get('http://localhost:3001/api/v1/hierarchy/members?sort=name&order=asc', {
      headers: {
        'Authorization': `Bearer ${agencyAToken}`
      }
    });
  ...
  test('visibility middleware handles filtering correctly', async ({ request }) => {
    ...
    const response = await request.get('http://localhost:3001/api/v1/hierarchy/members?tier=organization', {
      headers: {
        'Authorization': `Bearer ${agencyAToken}`
      }
    });
...
test.describe('Visibility Middleware Implementation', () => {
  test('visibilityCheckRaw.ts exists and has correct implementation', async ({ page }) => {
    ...
    const otherAgencies = data.members.filter((m: any) => 
      m.tier === 'agency' && m.email !== 'agency@example.com'
    );
    expect(otherAgencies.length).toBe(0);
  });
