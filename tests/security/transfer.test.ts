...
describe('Transfer Authorization', () => {
  ...
  beforeAll(async () => {
    ...
    org = await (global as any).createUser({
      tier: 'organization',
      parentAgencyId: agency.id,
      points: 5000,
      credits: 50,
    });
    admin = await (global as any).createUser({
      tier: 'admin',
      parentOrganizationId: org.id,
      points: 1000,
      credits: 10,
    });
    general1 = await (global as any).createUser({
      tier: 'general',
      parentOrganizationId: org.id,
      points: 500,
      credits: 5,
    });
    general2 = await (global as any).createUser({
      tier: 'general',
      parentOrganizationId: org.id,
      points: 500,
      credits: 5,
  ...
  describe('Invalid Transfers', () => {
    it('should reject transfer to user not visible', async () => {
      const otherOrg = await (global as any).createUser({
        tier: 'organization',
        parentAgencyId: 'other-agency',
      });
