...
test.describe('Authorization Verification', () => {
  ...
  test('authorization checks for password change', async ({ page, request }) => {
    ...
    const response = await request.put('http://localhost:3001/api/v1/users/' + someOtherUserId + '/password', {
      headers: {
        'Authorization': `Bearer ${generalToken}`
      },
      data: {
  ...
  test('authorization checks for tier elevation', async ({ page, request }) => {
    ...
    const response = await request.put('http://localhost:3001/api/v1/admin/users/' + generalUserId + '/tier', {
      headers: {
        'Authorization': `Bearer ${agencyToken}`
      },
      data: {
  ...
  test('authorization checks for notification settings', async ({ page, request }) => {
    ...
    const response = await request.put('http://localhost:3001/api/v1/admin/notifications/settings', {
      headers: {
        'Authorization': `Bearer ${orgToken}`
      },
      data: {
  ...
  test('authorization checks for user suspension beyond blocking', async ({ page, request }) => {
    ...
    const response = await request.post('http://localhost:3001/api/v1/admin/users/' + generalUserId + '/suspend', {
      headers: {
        'Authorization': `Bearer ${orgToken}`
      },
      data: {
