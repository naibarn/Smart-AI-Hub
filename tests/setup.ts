...
mockApp.use((req: any, res: any, next: any) => {
  ...
  if (
    (req.path === '/api/v1/auth/register' && req.method === 'POST') ||
...
mockApp.post('/api/v1/auth/login', async (req: any, res: any) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, token, user: { id: user.id, email: user.email, tier: user.tier } });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
mockApp.get('/api/v1/hierarchy/members', async (req: any, res: any) => {
  try {
    ...
    const members = Array.from(mockUsers.values()).filter(
      (u) => u.id !== user.id && (global as any).checkUserVisibility(user.id, u.id)
    );
...
(global as any).createUser = async (userData: any) => {
  ...
  const user = {
    ...
    email:
      userData.email ||
      (userData.tier
        ? `${userData.tier}-${Math.random().toString(36).substr(2, 9)}@example.com`
        : `general-${Math.random().toString(36).substr(2, 9)}@example.com`),
...
(global as any).getReferralStats = async (userId: string) => {
  ...
  const referralsCount = Array.from(mockUsers.values()).filter(
    (u) => u.invitedBy === userId && u.email !== referrer.email
  ).length;

  // Get referral rewards
  const rewards = Array.from(mockReferralRewards.values()).filter(
    (r) => r.referrerId === userId && r.status === 'completed'
  );
...
(global as any).checkUserVisibility = async (viewerId: string, targetId: string) => {
  ...
  if (viewer.tier === 'admin') {
    if (
      (target.tier === 'general' || target.tier === 'admin') &&
      target.parentOrganizationId === viewer.parentOrganizationId
    )
      return true;
  }
