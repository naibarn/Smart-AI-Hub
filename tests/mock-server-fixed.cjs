...
// Helper function to get menu visibility based on user tier
function getMenuVisibility(userTier) {
  return {
    'menu-dashboard': true,
    'menu-profile': true,
    'menu-points': true,
    'menu-referrals': true,
    'menu-members': ['agency', 'organization', 'admin', 'administrator'].includes(userTier),
    'menu-agency-settings': ['agency', 'administrator'].includes(userTier),
    'menu-transfer': ['agency', 'organization', 'admin', 'administrator'].includes(userTier),
    'menu-block-users': ['agency', 'organization', 'admin', 'administrator'].includes(userTier),
    'menu-org-settings': ['organization', 'administrator'].includes(userTier),
    'menu-admin-settings': ['administrator'].includes(userTier)
  };
}
...
function checkAccess(route) {
  return (req, res, next) => {
    ...
    try {
      ...
      console.log('Checking access for user:', user.tier, 'to route:', route);
      if (!checkRouteAccess(user.tier, route)) {
        console.log('Access denied for user:', user.tier, 'to route:', route);
  ...
  res.send(`
    ...
    <body>
      ...
      <div class="menu">
        ...
        <div class="menu-item" data-testid="menu-points">Points</div>
        <div class="menu-item" data-testid="menu-referrals">Referrals</div>
      ...
      <script>
        ...
        if (userData) {
          ...
          const menuVisibility = getMenuVisibility(userTier);
          
          // Apply menu visibility
          Object.keys(menuVisibility).forEach(menuId => {
            const element = document.querySelector(\`[data-testid="\${menuId}"]\`);
            if (element) {
              element.style.display = menuVisibility[menuId] ? 'block' : 'none';
            }
          });
          
          // Debug: Log menu visibility
          console.log('User tier:', userTier);
          console.log('Menu visibility:', menuVisibility);
          
          // Show member list if user has access
          if (menuVisibility['menu-members']) {
...
app.get('/members', checkAccess('/members'), (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
app.get('/agency/settings', checkAccess('/agency/settings'), (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
app.get('/transfer', checkAccess('/transfer'), (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
...
app.get('/block', checkAccess('/block'), (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
...
app.get('/organization/settings', checkAccess('/organization/settings'), (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
app.get('/admin/settings', checkAccess('/admin/settings'), (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
